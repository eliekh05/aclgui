...
use crate::ui::ace_editor::AceEditorMode;
...
pub fn draw(app: &mut AclApp, ui: &mut egui::Ui) {
    ...
    egui::ScrollArea::vertical().show(ui, |ui| {
        ...
        ui.group(|ui| {
            ...
            egui::Grid::new("info_grid").num_columns(2).spacing([16.0, 4.0]).show(ui, |ui| {
                ui.label("ACL type:");
        ...
        if let Some(mode) = &acl.posix_mode {
            ui.group(|ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    if mode.setuid { ui.label("setuid"); }
                    if mode.setgid { ui.label("setgid"); }
                    if mode.sticky { ui.label("sticky"); }
                });
        ...
        if !acl.aces.is_empty() || !acl.default_aces.is_empty() {
            ui.group(|ui| {
                ...
                if !acl.default_aces.is_empty() {
                    ui.add_space(6.0);
                    ui.label(egui::RichText::new("Default ACEs (inherited by new items)").italics());
        ...
        if let Some(ref result) = app.apply_result {
            ...
            ui.group(|ui| {
                match result {
                    Ok(msg) => {
                        ui.colored_label(egui::Color32::from_rgb(80, 200, 80), "✔ Applied successfully");
                        ui.label(msg);
                    }
                    Err(e) => {
...
fn draw_mode_grid(ui: &mut egui::Ui, mode: &PosixMode) {
    egui::Grid::new("mode_grid").num_columns(4).spacing([12.0, 2.0]).show(ui, |ui| {
        ui.label("");
        ui.label("Read");
        ui.label("Write");
        ui.label("Execute");
...
fn draw_ace_table(ui: &mut egui::Ui, aces: &[Ace], default: bool, app: &mut AclApp) {
    egui::Grid::new(if default { "ace_grid_default" } else { "ace_grid" })
        .num_columns(7)
        .striped(true)
        .spacing([8.0, 4.0])
        .show(ui, |ui| {
            ...
            for (i, ace) in aces.iter().enumerate() {
                ...
                let r = &ace.rights;
                ui.monospace(format!(
                    "{}{}{}", bit(r.read || r.list), bit(r.write || r.create_file), bit(r.execute)
                ));
                ...
                if ih.file_inherit || ih.object_inherit  { inh.push('f'); }
                if ih.dir_inherit  || ih.container_inherit { inh.push('d'); }
                if ih.inherited                           { inh.push_str("(i)"); }
                ui.label(inh);
            ...
            if let Some(idx) = remove_idx {
                app.staged.changes.push(Change::RemoveAce { index: idx, default });
                app.status = format!("Staged: remove ACE #{idx}. Apply when ready.");
            }
        });
}

fn bit(b: bool) -> char { if b { '●' } else { '○' } }
