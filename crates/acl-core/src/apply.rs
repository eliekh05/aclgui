...
use crate::model::*;
use crate::os_detect::{Os, ToolAvailability, current_os};
...
fn ace_to_macos_chmod_spec(ace: &Ace) -> Result<String, String> {
    ...
    if ace.rights.read        { perms.push("read"); }
    if ace.rights.write       { perms.push("write"); }
    if ace.rights.execute     { perms.push("execute"); }
    if ace.rights.delete      { perms.push("delete"); }
    if ace.rights.append      { perms.push("append"); }
    if ace.rights.create_file { perms.push("add_file"); }
    if ace.rights.create_dir  { perms.push("add_subdirectory"); }
    if ace.inherit.file_inherit { perms.push("file_inherit"); }
    if ace.inherit.dir_inherit  { perms.push("directory_inherit"); }
...
fn windows_ace_parts(ace: &Ace) -> Result<(&'static str, String, String), String> {
    ...
    if ace.inherit.object_inherit    { rights.push_str("(OI)"); }
    if ace.inherit.container_inherit { rights.push_str("(CI)"); }
    if ace.rights.read && ace.rights.write && ace.rights.execute
        && ace.rights.delete && ace.rights.write_security {
        rights.push('F');
        ...
        if ace.rights.read          { bits.push("RD"); }
        if ace.rights.write         { bits.push("WD"); }
        if ace.rights.execute       { bits.push("X"); }
        if ace.rights.delete        { bits.push("DE"); }
        if ace.rights.read_attr     { bits.push("RA"); }
        if ace.rights.write_attr    { bits.push("WA"); }
        if ace.rights.read_security { bits.push("RC"); }
        if ace.rights.write_security { bits.push("WDAC"); }
